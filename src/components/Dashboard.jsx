...
import { COLORS, POS_COLORS } from '../utils/constants'
import './Dashboard.css'

export default function Dashboard({ draws }) {
  const stats = useMemo(() => {
    ...
    draws.forEach(draw => {
      const [_, __, numbers] = draw
      numbers.forEach((num, idx) => {
        frequencyMap[num] = (frequencyMap[num] || 0) + 1
        positionFreq[idx][num] = (positionFreq[idx][num] || 0) + 1
      })
    })
    ...
    const bottomNumbers = Object.entries(frequencyMap)
      .sort((a, b) => a[1] - b[1])
      .slice(0, 10)
